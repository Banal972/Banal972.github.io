...
const fonts = [
  { name: 'Pretendard', data: readFont('Pretendard-Regular.otf'), weight: 400 as const, style: 'normal' as const },
  { name: 'Pretendard', data: readFont('Pretendard-Bold.otf'), weight: 700 as const, style: 'normal' as const },
];
...
export async function renderOgImage({ title, category, hue = 250, meta }: OgOptions) {
  const tree = h(
    'div',
    {
      width: '100%',
    ...
    [
      h(
        'div',
        { display: 'flex', alignItems: 'center', gap: '16px', fontSize: 26, fontWeight: 700, color: `hsla(${hue}, 92%, 80%, 1)` },
        category ?? SITE.title,
      ),
      h(
        'div',
      ...
      h(
        ...
        [
          h('div', { display: 'flex', fontWeight: 700, color: '#ffffff' }, SITE.title),
